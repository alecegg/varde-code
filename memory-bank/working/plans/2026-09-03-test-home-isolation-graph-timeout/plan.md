...
- [x] Given concurrent unit tests, when they override HOME, then every override is serialized and restored after its test.
- [x] Given the graph-count test, when it runs in the workspace suite, then it completes within the 120-second review budget.
- [x] Given the workspace test suite, when it runs, then it completes without HOME-derived configuration failures.
